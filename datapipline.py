...
class PairDataset(BaseDataset):
    ...
    def __getitem__(self, index): # 这个方法如何，何时调用？
...
def Get_paired_dataset(batch_size):
    data = PairDataset()
    opt = {"dataroot": "../final_dataset", "phase": "train", "resize_or_crop": False, "isTrain": True, "no_flip": True,
           "vary": 1, "lighten": True, "fineSize": 400}
    opt = Structure(opt)
    data.initialize(opt)
